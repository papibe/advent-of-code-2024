...
def get_radius(
    grid: Grid, srow: int, scol: int, cheat_radious: int
) -> Set[Tuple[int, int]]:
    cheat_desintations: Set[Tuple[int, int]] = set()

    for step_row in range(cheat_radious + 1):
        for step_col in range(cheat_radious - step_row + 1):
            for dir_row, dir_col in [(1, 1), (-1, 1), (-1, -1), (1, -1)]:
                new_row = srow + dir_row * step_row
                new_col = scol + dir_col * step_col

                if 0 <= new_row < len(grid) and 0 <= new_col < len(grid[0]):
                    if grid[new_row][new_col] != WALL:
                        cheat_desintations.add((new_row, new_col))
