...
func solve(reports [][]int) int {
	...
	for _, report := range reports {
		...
		for index := 0; index < len(report); index++ {
			simple_report := make([]int, len(report))
			copy(simple_report, report)
			simple_report = append(simple_report[:index], simple_report[index+1:]...)
...
func main() {
	...
	fmt.Println(solution("./input.txt"))   // 528
}
