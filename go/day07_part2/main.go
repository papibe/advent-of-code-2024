...
func eq_is_possible(test_value int, operands []int, current_value int) bool {
	...
	concat := strconv.Itoa(current_value) + strconv.Itoa(operands[0])
	operation, _ := strconv.Atoi(concat)
	concat_ok := eq_is_possible(test_value, operands[1:], operation)

	return plus_ok || mul_ok || concat_ok
}
...
func main() {
	fmt.Println(solution("./example.txt")) // 11387
	fmt.Println(solution("./input.txt"))   // 581941094529163
}
